...
- Рефакторинг: в конструктор com.sanefox.webapp.model.Resume добавить второй параметр `fullName`
- Сделать рефакторинг всех реализаций `Storage`: заменить метод `com.sanefox.webapp.model.Resume[] getAll()` на `List<com.sanefox.webapp.model.Resume> getAllSorted()`
